...
function showConnectOverlay() {
    ...
    // startText.innerHTML = 'start play';
...
function showPlayOverlay() {
    ...
    setOverlay('clickableState', img, event => {
        ...
        hideOverlay();
window.parent.uiCallback && window.parent.uiCallback()
...
function onWebRtcAnswer(webRTCData) {
    ...
    webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
        ...
        statsText += `Video Resolution: ${aggregatedStats.hasOwnProperty('frameWidth') && aggregatedStats.frameWidth && aggregatedStats.hasOwnProperty('frameHeight') && aggregatedStats.frameHeight ?
...
function updateVideoStreamSize() {
    ...
    if (now - lastTimeResized > 1000) {
        ...
        let descriptor = {
            "appname": "web",
            "cmd": "command",
            "msg": "r.SetRes " + window.innerWidth + "x" + window.innerHeight + "w"
        };
        emitUIInteraction(descriptor);
...
function connect() {
    ...
    ws = new WebSocket(UIConfig.ue.url.replace('http://', 'ws://').replace('https://', 'wss://'));
...
function checkBrowser() {
    ...
    console.info(navigator.userAgent + '<br><br>')
    if (mobile_browser.versions.mobile || mobile_browser.versions.ios || mobile_browser.versions.android || mobile_browser.versions.iPhone || mobile_browser.versions.iPad) {
        console.info('访问终端：移动端<br/>')
    } else {
        console.info('访问终端：非移动端<br/>')
    }
    let br = browser();
    console.info(JSON.stringify(br));
    if (!((br.chrome && br.chrome >= 90) || (br.safari && br.safari >= 14))) {
        alert('非最新版本的chrome内核浏览器，请更换为最新版本Chrome或Safari浏览器进行查看.');
        return false;
    }
    return true;
}
...
function myHandleResponseFunction(datamsg) {
    //alert(data);
    //console.log(data);
    var data = JSON.parse(datamsg);
    if (data.cmd == "rotation") {
        window.geoKingViewer.callEventListener(GeoKingViewer.EVENT_ROTATE, data.data);
    }

    if (data.cmd == "search_poi") {
        var poiMessage = {
            "type": data.type,
            "id": data.id,
        ...
        window.geoKingViewer.callEventListener(GeoKingViewer.EVENT_POI, poiMessage);
    }

    if (data.cmd == "search_bim") {
        // var poiMessage = {
        // 	"type": data.type,
        // 	"id": data.id,
        // 	"poiname": data.poiname,
        // 	"more": data.more
        // };
        window.geoKingViewer.callEventListener(GeoKingViewer.EVENT_BIM, data);
    }
 ...
 * 构造函数
 */
function GeoKingViewer() {
    if (isNaN(GeoKingViewer.seq))
        GeoKingViewer.seq = 0;
    else
        GeoKingViewer.seq++;
    this.id = "GeoKingViewer" + GeoKingViewer.seq;
    this.divId = "GeoKingViewer " + GeoKingViewer.seq;
    //this.width = w;
    //this.height = h;
    this.listeners = {};
}
 ...
 * 添加监听函数
 */
GeoKingViewer.prototype.addEventListener = function (type, fn) {
    if (!this.listeners[type]) {
        this.listeners[type] = [];
    }
    this.listeners[type].push(fn);

    if (type != GeoKingViewer.SYSTEM_JS_INIT) {
        //UE注册事件,暂时没做
    }
}
 ...
 * 注销监听函数
 */
GeoKingViewer.prototype.removeEventListener = function (type) {
    if (this.listeners[type]) {
 ...
 * 调用监听函数
 */
GeoKingViewer.prototype.callEventListener = function (type, param) {

    //panoviewer.jsTrace(this.listeners);
    if (this.listeners[type] == undefined) {
        return;
    }
    for (var i = 0; i < this.listeners[type].length; i++) {
        this.listeners[type][i](param);
    }
}

/***当JS初始化完成*/
GeoKingViewer.SYSTEM_JS_INIT = "SystemEvent.SYSTEM_JS_INIT";


/***当视口旋转时候派发
 *param{yaw}
 */
GeoKingViewer.EVENT_ROTATE = "GeoKingViewerEvent.sceneRotate";
 ...
 */
GeoKingViewer.EVENT_POI = "GeoKingViewerEvent.poisearch";
GeoKingViewer.EVENT_BIM = "GeoKingViewerEvent.bimsearch";


GeoKingViewer.prototype.clearConsole = function () {
    let descriptor = {
        "appname": "web",
        "cmd": "command",
        "msg": "DisableAllScreenMessages"
    };
    emitUIInteraction(descriptor);

}

GeoKingViewer.prototype.fps = function (visible) {

    let descriptor = {
        "appname": "web",
        "cmd": "command",
        "msg": "stat fps " + visible
    };
    emitUIInteraction(descriptor);

}

GeoKingViewer.prototype.setTime = function (timeValue) {
...
GeoKingViewer.prototype.flyTo = function (posName) {
...
GeoKingViewer.prototype.setWeather = function (weatherType) {
...
//yaw左右 pitch上下
GeoKingViewer.prototype.setRotation = function (yaw, pitch) {
...
GeoKingViewer.prototype.setLayerVisble = function (levelName, visible) {
...
GeoKingViewer.prototype.setHotmapVisible = function (visible) {
...
GeoKingViewer.prototype.setHotmapVal = function (val) {
...
GeoKingViewer.prototype.MoveTo = function (posIndex) {
...
GeoKingViewer.prototype.setSectionHigh = function (highValue) {
...
GeoKingViewer.prototype.setCesium = function (url, tag, visible, height=0, closeall=false) {
...
// 设置可见性
// levelName: "L1,L2,L3" or levelName:"L1"
GeoKingViewer.prototype.setLayer = function (levelName, visable) {
    let descriptor = {
        "appname": "web",
        "cmd": "setLayer",
        "levelName": levelName,
...
//
var geoKingViewer = new GeoKingViewer();
