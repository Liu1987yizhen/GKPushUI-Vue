import $ from "jquery"
// param为传入参数
function packageFunc(url) {
    if (window._CONFIG && window._CONFIG.dev) {
        return window._CONFIG.config_dev + url ? "." + url : ""
    } else {
        return window._CONFIG.config_dev + url ? "." + url : ""
    }
}
...
export { moveDiv, errroCatch, currentExtent, viewCenter, saveCurViewerImage, downloadFile, pointVisibilityOnEarth, CVT, CursorTip, checkComponent, checkViewer }
export default {
    moveDiv,
    errroCatch,
    currentExtent,
    ...
    install: function(Vue) {
        Vue.prototype.glob_dev = (url) => packageFunc(url)
    }
}
